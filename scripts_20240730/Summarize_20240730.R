 ...
 #  -----------------------------------------------------------------------------------------------------------
  ...
  
  {
    ...
    for(i in 1:length(Species.list)){
      ...
      df.temp = df.all.filtered[df.all.filtered$Species == Species.list[i],]
      pdf(file.path(displacements.dir, paste0((paste(Species.list[i], "displace_filtered", sep="_")), ".pdf")),  width = 25, height = 20)
      breaks <- seq(min(df.temp$speed.meanBL.all), max(df.temp$speed.meanBL.all)+0.001, by = 0.001)
      h <- hist(df.temp$speed.meanBL.all, breaks = breaks, prob = TRUE,
      ...
      density_est <- density(df.temp$speed.meanBL.all) #adjust bw for sensitivity
      min_peak_height <- 0.05
...

{
  ...
  write.csv(df.sum_mean_duration, file.path(other.place, "durations_summarize_all_norm.csv"), row.names=FALSE)
  ...
  write.csv(df.mean_duration_spec, file.path(other.place, "durations_summarize_spec_norm.csv"), row.names=FALSE)
...
#mean speed
sink(file.path(other.place,"mean_speed_norm_LMM.txt"))
...
sink(file.path(other.place,"pause_sec_LMM.txt"))
...
sink(file.path(other.place,"mean_speed_move_norm_LMM.txt"))
...

{
  ...
  print(move_sum.box)
  ggsave(file.path(move_pause.place, "move_sum_box.pdf"), move_sum.box, width = 6, height = 5.5, units = "cm", device='pdf', dpi=600)
  ...
  print(pause_sum.box)
  ggsave(file.path(move_pause.place, "pause_sum_box.pdf"), pause_sum.box, width = 6, height = 5.5, units = "cm", device='pdf', dpi=600)
