...
# Setup -------------------------------------------------------------------
{
  ...
  Species.info <- data.frame(
    ...
    Other_name = c("Z.nevadensis", "N.sugioi", "H.sjostedti", "C.formosanus", "O.formosanus" )
  )
  
  #Species list
  Species.list <- list.files("data_raw")
...
{
  {
    ...
    for(i in 1:length(Species.list)){
      # get scale data (pixels per plate diameter (=14.5 mm) 
      # and BL) from file "res.csv" created by videoScaleBL.py
      scaledata= list.files(file.path("data_raw", Species.list[i]), 
                            full.names = TRUE, pattern = "res.csv")
      ...
      rawdata = list.files(file.path("data_raw", Species.list[i]), 
                           full.names = TRUE, pattern = "position.csv")
      dataname = list.files(file.path("data_raw", Species.list[i]),
                            full.names = FALSE, pattern = "position.csv")
    
      if(length(rawdata) == 0){ next; }
      
      for(v in 1:length(dataname)){
        # Video information
        Genus = Species.info[Species.info[,3] == Species.list[i], 1]
        Specific = sub("[A-Za-z0-9]*.", "", Species.list[i])
        Species = paste(Genus, Specific, sep="_")
        Colony = sub("\\-[0-9]*-position.csv", "", dataname[v])
        Rep = sub(".*-(.*)-.*", "\\1", dataname[v])
        
        Name = paste(Colony, Rep, sep="_")
        ...
        print(paste("Species: ", i, "/", length(Species.list), 
                    ";  File: ",  v, "/", length(dataname), "->", Name))
        ...
        if(Plot){
          rawtraj_dir= file.path("output/raw-trajectories", Species.list[i])
          ...
          rawtrajplot <- ggplot() +
            ...
            labs(x = "x", y = "y")
          ggsave(filename=paste0(Name, ".png"), path=rawtraj_dir, 
                 width = 5, height = 5, device='png', dpi=600)
        }
...
trajBL_dir <- "output/BL-trajectories"
...
# plot BL vs. trajectory length-------------------------------------------------------------
{
  ...
  if(BLTraj_Plot){
    ...
    print(trajplot)
    ggsave(filename="BL-stdtrajL_nestcol.png", path=trajBL_dir, 
           width = 5, height = 5, device='png',  dpi = 600)
    ggsave(filename="BL-stdtrajL_nestcol.pdf", path=trajBL_dir, 
  ...
  if(BLTraj_Plot){
    trajplot<- ggplot(df.traj, aes(x=BL, y=trajlen, col=Nesting)) +
      ...
      theme(plot.title = element_text(hjust=0.5),
            plot.margin = unit(c(1, 1, 1, 1), "cm"))
    print(trajplot)
    ggsave(filename="BL_trajL_nestcol.png", path=trajBL_dir, 
           width = 5, height = 5, device='png',  dpi = 600)
    ggsave(filename="BL_trajL_nestcol.pdf", path=trajBL_dir, 
           width = 5, height = 5, device='pdf',  dpi = 600)
  }
  ...
  if(TrajBL_Plot){
    ...
    print(trajplot)
    ggsave(filename="BL-stdtrajL_speccol.pdf", path=trajBL_dir, 
           width = 8, height = 7, device='pdf', dpi=600)
  }
  ...
  if(BLTraj_Plot){
    trajplot<- ggplot(df.traj, aes(x=BL, y=trajlen, col=Species)) +
      ...
      theme(plot.title = element_text(hjust=0.5),
            plot.margin = unit(c(1, 1, 1, 1), "cm"))
    print(trajplot)
    ggsave(filename="BL_trajL_speccol.png", path=trajBL_dir, 
           width = 5, height = 5, device='png', dpi=600)
  }
...
# plot trajectory length box plots-------------------------------------------------------------
{
  ...
  trajbox_dir <- "output/trajectory_boxplots"
  ...
  if(Traj_boxPlot){
    nestplot<- ggplot(df.traj, aes(x=Nesting, y=trajlen.bl, col=Nesting)) +
      ...
      theme(plot.title = element_text(hjust=0.5),
            plot.margin = unit(c(1, 1, 1, 1), "cm"))
    print(nestplot)
    ggsave(filename="nest-stdtrajL_box_nestcol.pdf", path=trajbox_dir, 
           width = 5, height = 5, device='pdf',  dpi = 600)
  }
