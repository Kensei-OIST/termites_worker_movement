...
# Setup -------------------------------------------------------------------
{
  ...
  # Load data
  load("data_fmt/Pause_Threshold.rda")
  load("data_fmt/Ind_data_norm.rda")
  load("data_fmt/Duration_norm.rda")
...
{
  sink("output/Traveled.dis_norm_Anova_step_maxpeak02plus01.txt")
  
  df.ind.data$Forager <- df.ind.data$Nesting != "OP"
  r <- lmer(Traveled.dis.bl ~ Forager + (1|Species/Colony), data=df.ind.data)
  Anova(r)
  summary(r)
...
# Fitting ------
{
  ...
  
  {
    ...
    for(species in 1:length(Species.List)){
      for(behavior in 1:2){
        ...
        param_SE <- optim(c(0.1,0.1), SE_bin_LLF, data=StepData, 
                          xmin=min_sec, control = list(fnscale = -1), method="Nelder-Mead")
        ...
        ## result plot
        if(Plot_behav){
          png(paste0("output/fitting/", (paste(Species, Behav, sep="_")), "_step_maxpeak02plus01.png"))
          plot(Step_lab,Step_cum, log="xy",axes=F,
               ylab="inverse cumulative frequency", xlab="time",
               ylim=c(0.00001,1), xlim=c(min_sec, max_sec),
        ...
        if(Plot_behav_PDF){
          pdf(paste0("output/fitting/", (paste(Species, Behav, sep="_")), 
                     "_step_maxpeak02plus01.pdf"), width = 5, height = 4)
          plot(Step_lab,Step_cum, log="xy",axes=F,
               ylab="inverse cumulative frequency", xlab="time",
               ylim=c(0.00001,1), xlim=c(min_sec, max_sec))
  ...
  save(df.fitting.res, file = "data_fmt/fitting_res.rda")
  save(df.behav.all, file = "data_fmt/fitting_res_all.rda")
  
  
  sink("output/fitting_TPSE_speed.meanBL_step_maxpeak02plus01.txt")
  df.fitting.res
  sink()
}
...
if(Plot_behav.all){
  ...
  # move_all plot
  png(file.path(movement.dir, "move_all_step_maxpeak02plus01.png"),width = 240, height = 300, units='mm',res=600)
  ...
  # pause_all plot
  png(file.path(movement.dir, "pause_all_step_maxpeak02plus01.png"), width = 240, height = 300, units='mm',res=600)
...
if(Plot_behav.all_PDF){
  ...
  print(move_all.p)
  ggsave(file.path(movement.dir, "move_all_step_maxpeak02plus01.pdf"), width = 10, height = 5.5, units='cm', device='pdf', dpi=600)
  ...
  print(stop_all.p)
  ggsave(file.path(movement.dir, "stop_all_step_maxpeak02plus01.pdf"), width = 10, height = 5.5, units='cm', device='pdf', dpi=600)
}
