...
{
  sink(file.path(other.place, "Traveled.dis_norm_Anova_step_maxpeak02plus01.txt"))
...

{
  ...
  for(species in 1:length(Species.List)){
    for(behavior in 1:2){
      ...
      ## result plot
      if(Plot_behav){
        png(file.path(movement.dir, "move_pause", paste0((paste(Species, Behav, sep="_")), "_step_maxpeak02plus01.png")))
      ...
      if(Plot_behav_PDF){
        pdf(file.path(movement.dir, paste0((paste(Species, Behav, sep="_")), "_step_maxpeak02plus01.pdf")), width = 5, height = 4)
        plot(Step_lab,Step_cum, log="xy",axes=F,
             ylab="inverse cumulative frequency", xlab="time", ylim=c(0.00001,1), xlim=c(min_sec, max_sec))
...
sink(file.path(other.place,"fitting_TPSE_speed.meanBL_step_maxpeak02plus01.txt"))
df.fitting.res
sink()
